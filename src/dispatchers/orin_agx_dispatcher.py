...
class OrinAGXDispatcher:
    ...
    def __init__(self, dummy=False):
        ...
        if dummy:
            for k in self.file_associations.keys():
                self.file_associations[k] = (
                    f"../dummy_files/orin_nano/{self.file_associations[k]}"
                )
    ...
    def get_power_figs(self, metrics, subsystems=["all"]):
        ...
        related_files = sorted(
            glob.glob(self.file_associations["voltage"].replace("?", "*"))
        )
        ...
        for ch_hw in channel_hwmons:
            ...
            for idx in range(1, 4):
                ...
                voltage = ""
                current = ""
                curr_path_suffix = os.path.split(self.file_associations["current"])[
                    -1
                ].replace("?", str(idx))
                volt_path_suffix = os.path.split(self.file_associations["voltage"])[
                    -1
                ].replace("?", str(idx))
                name_path_suffix = (
                    os.path.split(self.file_associations["voltage"])[-1]
                    .replace("?", str(idx))
                    .replace("input", "label")
                )
                curr_path = os.path.join(
                    abs_path_prefix, path_prefix, ch_hw_path, curr_path_suffix
                )
                volt_path = os.path.join(
                    abs_path_prefix, path_prefix, ch_hw_path, volt_path_suffix
                )
                name_path = os.path.join(
                    abs_path_prefix, path_prefix, ch_hw_path, name_path_suffix
                )
                ...
                ret[name] = {
                    "voltage": f"{voltage} {self.units['voltage']}",
